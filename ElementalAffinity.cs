...
public class ElementalAffinity : MonoBehaviour {
	...
	public float calcElementalDamage(Element source, Element target) {
		switch(source) {
		case Element.None:
			return 1.0f;
		case Element.Water:
	...
	private float calcNormal(Element source, Element target) {
	...
	private float calcLD(Element source, Element target) {
